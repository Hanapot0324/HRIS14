...
app.get('/api/department-assignment/:id', (req, res) => {
  ...
  db.query('SELECT * FROM department_assignment WHERE id = ?', [id], (err, result) => {
    if (err) return res.status(500).send(err);
    if (result.length === 0) return res.status(404).send('Department Assignment not found');
    res.json(result[0]);
  });
});
...
app.post('/api/department-assignment', (req, res) => {
  ...
  if (!code || !employeeNumber) return res.status(400).send('Code and Employee Number are required');
...
app.get("/api/payroll-with-remittance", (req, res) => {
  ...
  SELECT
    ...
    CONCAT(pt.lastName, ', ', CONCAT_WS(' ', pt.firstName, pt.middleName, pt.nameExtension)) AS name,
...
const [lastNamePart, firstMiddlePart] = name.split(',');
const lastName = lastNamePart.trim();

const nameParts = firstMiddlePart.trim().split(' ');
const firstName = nameParts[0] || '';
const nameExtensionCandidates = ['Jr.', 'Sr.', 'II', 'III', 'IV']; // Extend as needed
...
for (let i = 1; i < nameParts.length; i++) {
  if (nameExtensionCandidates.includes(nameParts[i])) {
    ...
    middleNameParts.push(nameParts[i]);
  }
}
...
const middleName = middleNameParts.join(' ');
  ...
  db.query(query, values, (err, result) => {
    ...
    db.query(personQuery, [firstName, middleName, lastName, nameExtension, employeeNumber], (err2, res) => {
      if (err2) {
